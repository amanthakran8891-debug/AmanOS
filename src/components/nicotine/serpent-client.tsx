...
export function SerpentClient({
  battle,
  report,
  recent,
}: {
  battle: SerpentBattle;
  report: NicotineReport;
  recent: any[];
}) {
  ...
  const free = `${Math.floor(sec / 86400)}d ${String(
    Math.floor((sec % 86400) / 3600)
  ).padStart(2, "0")}h ${String(Math.floor((sec % 3600) / 60)).padStart(
    2,
    "0"
  )}m ${String(sec % 60).padStart(2, "0")}s`;
  ...
  return (
    <div className="space-y-4">
      <div
        ...
        style={{
          background: `radial-gradient(120% 90% at 50% 0%, ${s.color}22, rgba(13,19,34,0.7) 60%)`,
        }}
      >
        ...
        <SerpentGlyph
          ...
          eye={s.eye}
          color={s.color}
          heavySmoke={s.heavySmoke}
          warningPulse={s.warningPulse}
        />
        ...
        <div className="mt-3">
          <div className="flex items-end justify-between">
            ...
            <span className="font-mono text-sm font-bold" style={{ color: s.color }}>
              {s.hpCurrent} / {s.hpMax} · {s.threatPct}%
            </span>
          </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div className="rounded-xl border border-line bg-surface-2/60 p-3">
            ...
            <p className="mt-0.5 font-mono text-sm font-bold tabular-nums text-neon-green">
              {free}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-line bg-surface-2/60 p-3">
            ...
            <p className="mt-0.5 text-lg font-black text-neon-green">
              −{battle.damage.total} HP
            </p>
          </div>
        </div>

        {battle.damage.healedToday > 0 && (
          <p className="mt-2 text-center text-[11px] font-semibold text-neon-red">
            ⚠ Nicotine use today healed the serpent +{battle.damage.healedToday} HP.
          </p>
        )}
      </div>
            ...
            >
              ...
              <span
                className={`flex-1 text-sm font-semibold ${
                  it.done ? "text-neon-green" : "text-white"
                }`}
              >
                {it.label}
              ...
              <span
                className={`text-sm font-bold ${
                  it.done ? "text-neon-green" : "text-slate-500"
                }`}
              >
                −{it.hp} HP
              </span>
      ...
      {battle.analyticsUnlocked ? (
        ...
        <div className="card">
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            Keep logging — advanced analytics unlock as data builds.
          </p>
            ...
            {battle.locked.map((l) => (
              ...
              >
                ...
                <p className="text-[11px] text-slate-500">
                  {l.locked
                    ? `Unlocks in ${l.unlocksInDays} more day${
                        l.unlocksInDays === 1 ? "" : "s"
                      }`
                    : "Unlocked"}
                </p>
      ...
      <div className="card">
        ...
        {battle.intel.enough ? (
          ...
          <p className="mt-2 text-sm text-slate-400">
            Not enough data yet — insights appear once you&apos;ve logged about a
            week of cravings and use.
          </p>
        )}
      </div>
...
function NicotineMissions({ board }: { board: SerpentBattle["missions"] }) {
  ...
  const act = (m: SerpentBattle["missions"]["missions"][number]) => {
    ...
    if (m.action === "gym") {
      start(() => void completeMission("gym"));
    } else if (m.action === "craving-won") {
      start(() => void logNicotineEvent({ type: "craving", outcome: "won" }));
    }
  };
    ...
    <div className="card">
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-bg">
        <div
          ...
          style={{
            width: `${progress}%`,
            boxShadow: "0 0 10px rgba(52,245,197,0.6)",
          }}
        />
      </div>
        ...
        {board.missions.map((m) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p
                className={`text-sm font-bold ${
                  m.done ? "text-neon-green" : "text-white"
                }`}
              >
                {m.label}
              </p>
              <p
                ...
                style={{ color: m.rewardKind === "hp" ? "#fb7185" : "#a78bfa" }}
              >
                {m.reward}
              </p>
            </div>
            ...
            {m.done ? (
              <span className="grid h-8 w-8 place-items-center rounded-full bg-neon-green text-bg">
                ✓
              </span>
            ) : m.action === "passive" ? (
...
function CostCard({ cost }: { cost: SerpentBattle["cost"] }) {
  ...
  return (
    <div className="card">
      ...
      <p className="mt-0.5 text-[11px] text-slate-500">
        Projected from the assumptions below — not logged spend. The Finance page
        shows logged transactions only.
      </p>

      <div className="mt-2 rounded-xl border border-line bg-surface-2/50 px-3 py-2 text-[11px] text-slate-400">
        Assumptions: {cost.assumptions.cigsPerDay} cigarettes/day · {c}
        {cost.assumptions.pricePerUnit}/unit · {c}
        {cost.assumptions.pricePerPack}/pack of {cost.assumptions.packSize}
      </div>
      ...
      <div className="mt-2 flex justify-between text-[12px]">
        <span className="text-slate-400">
          ...
          <span className="font-bold text-neon-green">
            {c}
            {cost.savedSoFar.toLocaleString()}
          </span>
        </span>
        <span className="text-slate-400">
          Spent (logged):{" "}
          <span className="font-bold text-slate-200">
            {c}
            {cost.spentLogged.toLocaleString()}
          </span>
        </span>
      </div>
    </div>
  ...
  return (
    <div className="card">
      ...
      <div className="mt-2 h-2 overflow-hidden rounded-full bg-bg">
        <div
          className="h-full rounded-full bg-gradient-to-r from-neon-amber to-neon-green"
