...
export function RecoveryTimelineCard({ timeline }: { timeline: RecoveryTimeline }) {
  ...
  const events = useMemo(() => withinDays(timeline.events, days, now).slice().reverse(), [timeline.events, days, now]);
  ...
  return (
    <div className="card">
      ...
      <div className="mt-3 max-h-72 space-y-1.5 overflow-y-auto pr-1">
        {events.length === 0 && <p className="text-sm text-slate-500">No events in this window.</p>}
        {events.map((e, i) => (
